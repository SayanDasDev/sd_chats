...
import ContactProfileDialog from "./contact-profile-dialogue";
...
export function ChatCard(props: ChatCardProps) {
  return (
    <Dialog>
      <ContextMenu>
        <ContextMenuTrigger>
          <Card className="w-full cursor-pointer select-none group hover:scale-95 transition-transform ease-linear duration-150 flex flex-col items-center justify-between px-4 py-6 relative">
            {props.state === "new" ? (
              <Badge
            ...
            <CardHeader className="px-2 pt-2 pb-0 flex flex-col items-center">
              ...
              <Badge
                variant="outline"
                className="font-thin text-center select-none w-fit text-foreground/70 capitalize"
              >
                last seen yesterday
              </Badge>
      ...
      <ContactProfileDialog name={props.sender} avatar={props.avatar} />
    </Dialog>
  );
}
